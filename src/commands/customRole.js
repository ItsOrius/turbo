  ...
  getCustomRoleData(interaction.guildId, interaction.user.id).then(customRole => {
    ...
    passesChecks(interaction.guildId, name, submittedName, submittedIcon).then(checks => {
      ...
      if (checks.includes(CHECKS.NAMES_DISABLED)) {
        ...
        interaction.reply({ embeds: [quickEmbed("Invalid Input", CHECKS.NAMES_DISABLED, Discord.Colors.Red)], ephemeral: true }).catch(console.log);
        return;
      }
      if (checks.includes(CHECKS.ICONS_DISABLED)) {
        // runs when role icons are disabled
        interaction.reply({ embeds: [quickEmbed("Invalid Input", CHECKS.ICONS_DISABLED, Discord.Colors.Red)], ephemeral: true }).catch(console.log);
        return;
      }
      if (checks.includes(CHECKS.ALPHANUMERIC)) {
        // runs when role name is not alphanumeric
        interaction.reply({ embeds: [quickEmbed("Invalid Name", CHECKS.ALPHANUMERIC, Discord.Colors.Red)], ephemeral: true }).catch(console.log);
        return;
      }
      if (checks.includes(CHECKS.UNSUPPORTED_NAME)) {
        // runs when role name is not supported
        interaction.reply({ embeds: [quickEmbed("Invalid Name", CHECKS.UNSUPPORTED_NAME, Discord.Colors.Red)], ephemeral: true }).catch(console.log);
        return;
      }
      if (checks.includes(CHECKS.UNSUPPORTED_ICON)) {
        // runs when role icon is not supported
        interaction.reply({ embeds: [quickEmbed("Invalid Icon", CHECKS.UNSUPPORTED_ICON, Discord.Colors.Red)], ephemeral: true }).catch(console.log);
        return;
      }
    ...
    passesChecks(interaction.guildId, name, submittedName, submittedIcon).then(checks => {
      ...
      interaction.reply({ embeds: [quickEmbed("Error", "An error occured while checking your role!", Discord.Colors.Red)], ephemeral: true }).catch(console.log);
    });
  });
}
